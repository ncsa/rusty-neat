...
- **Default model has no BND / INV.** ~~The bundled gnomAD-SV-derived `default_sv_model()` filters BND / INV out at training time.~~ **Resolved in v1.12.1 (#217)** via `tools/inject_cancer_sv_model.py`: the bundled COSMIC tumor model (`tools/cosmic_v104_pancancer_model.json.gz`) now carries a literature-derived `sv_model` covering all six SV types. The germline-side `default_sv_model()` was already populated at heuristic rates. A data refit from real cancer-SV calls is tracked at **#218** for v1.13.
...
- **Cancer-specific SvModel defaults.** Shipped in v1.12.1 (#217) as a literature-derived (PCAWG, Li et al. 2020) `sv_model` injection into the bundled COSMIC tumor model — see `tools/inject_cancer_sv_model.py`. The germline-side `default_sv_model()` stays gnomAD-derived. A data refit from PCAWG consensus SV calls or COSMIC StructVar is the v1.13 plan, tracked at **#218**.
